...
if (isset($_POST['rec'])) {
    $id = $_GET['id'];
$query = "UPDATE user set activo = '3' where id = '$id'";
        $result=$mysqli->query($query);
    if (!$result) {
    	die("Error");
    }?>
    <script>
      Swal.fire({
        type: 'success',
        title: 'Rechazado',
        text: 'Se Rechazado la Solicitud',
...
<div class="pt-4">
  ...
  <cener>Contactar a <b>'.$row["nick"].'</b> para notificarle a ha sido rechazada su solicitud!<br>
