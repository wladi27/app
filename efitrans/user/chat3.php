<?php
 ...
 if (!isset($_SESSION['nick_act'])) {
	...
	$query = "SELECT * FROM ofertas_v WHERE tipo = '0' AND recibe = '".$_SESSION['nick_act']."' ORDER by fecha DESC";
...
?>
	<div id="datos-chat" class="card" style="border-radius: 5%;">
        ...
        <span style="color: black;">Fecha de Envio:<b> <?php echo $fila['fecha']; ?></b></span>
				<form action="d_vl.php?id_oferta=<?php echo $fila['id_oferta']; ?>" method="post">
					<input type="submit" class="btn btn-danger btn-block" name="rec" value="Rechazar" class="button"/>

        </form >
        <form class="pt-1" action="c_vl.php?id_oferta=<?php echo $fila['id_oferta']; ?>" method="post">
